...
import (
	...
	"github.com/grafeas/grafeas/samples/server/name"
...
func (g *Grafeas) CreateNote(n *api.Note) *errors.AppError {
	if n.Name == "" {
...
func (g *Grafeas) CreateOccurrence(o *api.Occurrence) *errors.AppError {
	if o.Name == "" {
...
func (g *Grafeas) CreateOperation(o *api.Operation) *errors.AppError {
	if o.Name == "" {
		log.Printf("Invalid occurrence name: %v", o.Name)
		return &errors.AppError{Err: "Invalid occurrence name", StatusCode: http.StatusBadRequest}
	}
...
func (g *Grafeas) GetNote(pID, nID string) (*api.Note, *errors.AppError) {
	return g.S.GetNote(pID, nID)
}

// GetOccurrence gets a occurrence from the datastore.
func (g *Grafeas) GetOccurrence(pID, oID string) (*api.Occurrence, *errors.AppError) {
	return g.S.GetOccurrence(pID, oID)
}

// GetOccurrence gets a occurrence from the datastore.
func (g *Grafeas) GetOperation(pID, oID string) (*api.Operation, *errors.AppError) {
	return g.S.GetOperation(pID, oID)
}

// GetOccurrenceNote gets a the note for the provided occurrence from the datastore.
func (g *Grafeas) GetOccurrenceNote(pID, oID string) (*api.Note, *errors.AppError) {
...
func (g *Grafeas) UpdateNote(pID, nID string, n *api.Note) (*api.Note, *errors.AppError) {
	// get existing note
...
func (g *Grafeas) UpdateOccurrence(pID, oID string, o *api.Occurrence) (*api.Occurrence, *errors.AppError) {
...
func (g *Grafeas) UpdateOperation(pID, oID string, o *api.Operation) (*api.Operation, *errors.AppError) {
...
func (g *Grafeas) ListOperations(pID, fs string) (*api.ListOperationsResponse, *errors.AppError) {
	// TODO: support filters
	ops := g.S.ListOperations(pID, fs)
	return &api.ListOperationsResponse{Operations: ops}, nil
}

func (g *Grafeas) ListNotes(pID, fs string) (*api.ListNotesResponse, *errors.AppError) {
	// TODO: support filters
	ns := g.S.ListNotes(pID, fs)
	return &api.ListNotesResponse{Notes: ns}, nil

}

func (g *Grafeas) ListOccurrences(pID, fs string) (*api.ListOccurrencesResponse, *errors.AppError) {
	// TODO: support filters - prioritizing resource url
	os := g.S.ListOccurrences(pID, fs)
	return &api.ListOccurrencesResponse{Occurrences: os}, nil
}

func (g *Grafeas) ListNoteOccurrences(pID, nID, fs string) (*api.ListNoteOccurrencesResponse, *errors.AppError) {
	// TODO: support filters - prioritizing resource url
	os, err := g.S.ListNoteOccurrences(pID, nID, fs)
	if err != nil {
		return nil, err
	}
	return &api.ListNoteOccurrencesResponse{Occurrences: os}, nil
}
