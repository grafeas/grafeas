...
type memStore struct {
	occurrencesByID map[string]api.Occurrence
	notesByID       map[string]api.Note
	opsByID         map[string]api.Operation
}

// NewMemStore creates a memStore with all maps initialized.
func NewMemStore() server.Storager {
	return &memStore{make(map[string]api.Occurrence), make(map[string]api.Note),
		make(map[string]api.Operation)}
}

// CreateOccurrence adds the specified occurrence to the mem store
func (m *memStore) CreateOccurrence(o *api.Occurrence) *errors.AppError {
...
func (m *memStore) UpdateOccurrence(pID, oID string, o *api.Occurrence) *errors.AppError {
	oName := name.OccurrenceName(pID, oID)
	if _, ok := m.occurrencesByID[oName]; !ok {
		return &errors.AppError{Err: fmt.Sprintf("Occurrence with oName %q does not Exist", oName),
			StatusCode: http.StatusBadRequest}
...
func (m *memStore) GetOccurrence(pID, oID string) (*api.Occurrence, *errors.AppError) {
	oName := name.OccurrenceName(pID, oID)
...
func (m *memStore) ListOccurrences(pID, filters string) []api.Occurrence {
	os := []api.Occurrence{}
...
func (m *memStore) CreateNote(n *api.Note) *errors.AppError {
...
func (m *memStore) UpdateNote(pID, nID string, n *api.Note) *errors.AppError {
	nName := name.NoteName(pID, nID)
	if _, ok := m.notesByID[nName]; !ok {
		return &errors.AppError{Err: fmt.Sprintf("Note with name %q does not Exist", nName),
			StatusCode: http.StatusBadRequest}
...
func (m *memStore) GetNote(pID, nID string) (*api.Note, *errors.AppError) {
	nName := name.NoteName(pID, nID)
...
func (m *memStore) GetNoteByOccurrence(pID, oID string) (*api.Note, *errors.AppError) {
	oName := name.OccurrenceName(pID, oID)
	o, ok := m.occurrencesByID[oName]
	if !ok {
		return nil, &errors.AppError{Err: fmt.Sprintf("Occurrence with name %q does not Exist", oName),
...
func (m *memStore) ListNotes(pID, filters string) []api.Note {
	ns := []api.Note{}
...
func (m *memStore) ListNoteOccurrences(pID, nID, filters string) ([]api.Occurrence, *errors.AppError) {
	// TODO: use filters
...
func (m *memStore) GetOperation(pID, opID string) (*api.Operation, *errors.AppError) {
...
func (m *memStore) CreateOperation(o *api.Operation) *errors.AppError {
...
func (m *memStore) UpdateOperation(pID, opID string, op *api.Operation) *errors.AppError {
	opName := name.OperationName(pID, opID)
	if _, ok := m.opsByID[opName]; !ok {
		return &errors.AppError{Err: fmt.Sprintf("Operation with name %q does not Exist", opName),
			StatusCode: http.StatusBadRequest}
...
func (m *memStore) ListOperations(pID, filters string) []api.Operation {
	ops := []api.Operation{}
